...
INPUT_FILE  = rf"{BASE_DIR}\REEdeposit_PaleoCoords0907_4.csv"
OUTPUT_FILE = rf"{BASE_DIR}\REEdeposit_SubductionDistance0907_4.csv"
...
# ====== 排除特定 trench（解决 Noril=29 km 的 Mongol-Okhotsk 问题） ======
# key 用 interval_tag(age) 的格式，比如 "410-250"
EXCLUDE_PLATE_PAIRS_BY_INTERVAL = {
    "410-250": {(401, 0), (0, 401)},  # Mongol-Okhotsk subducting beneath Siberia
}
EXCLUDE_NAME_KEYWORDS_BY_INTERVAL = {
    "410-250": ["mongol-okhotsk"],    # 双保险：按名字再排一次
}

def keep_subduction_feature(feat: pygplates.Feature, itag: str) -> bool:
    """
    True = 保留该 feature 参与距离计算
    False = 排除
    """
...
def nearest_subduction_feature(lat, lon, subd_feats, rot_model, age):
    """返回最近 trench: (km, feature, name, pid, cid)"""
    pt = pygplates.PointOnSphere(lat, lon)
    reconstructed = []
    pygplates.reconstruct(subd_feats, rot_model, reconstructed, age)
...
for _, row in df.iterrows():
    ...
    # 缓存 subd_feats + rot_model（按 interval 缓存）
